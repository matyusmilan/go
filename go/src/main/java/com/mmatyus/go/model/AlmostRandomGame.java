...
public class AlmostRandomGame implements Game<Integer> {
  public static final int MAX_RANDOM_TRIES = 50;
  Board                   board;
  Random                  rand             = new Random();
  PlayerPolicy            policy;

  public AlmostRandomGame( BoardType boardType, PlayerPolicy policy ) {
  ...
  protected int selectRandomMoveInEndGame() {
    ...
    for( int tries = 0; tries < 3; ++tries ) {
      ...
      int l = board.emptyCells.nextSetBit( 0 );
      for( int i = 0; i < k; ++i ) {
        l = board.emptyCells.nextSetBit( l + 1 );
      }
  ...
  protected boolean isTrivialEye( int pos, int color ) {
    ...
    for( int ni = 0; ni < BoardType.MAX_NEIGHBORS; ++ni ) {
      ...
      if( board.cells[p] == Board.EMPTY || board.cells[p] == otherColor ) {
        ...
        if( board.numberOfLifes[board.shapeAtPos[p]] == 1 ) {
          return false;
        }
      }
    }
